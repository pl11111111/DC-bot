...
class NewMessageLog(commands.Cog):
    ...
    async def channel_deleted(self,channel_id):
        ...
        rows=await db.query("SELECT id FROM orders WHERE (channel_id=%s OR source_id=%s) AND status NOT IN ('completed','cancelled','refunded')",(channel_id,channel_id))
        if rows:
    ...
    async def cleanup(self):
        try:
            async with self.lock:
                ...
                safe="c.hold=FALSE AND NOT EXISTS (SELECT 1 FROM orders o WHERE (o.channel_id=c.channel_id OR o.source_id=c.channel_id) AND o.status NOT IN ('completed','cancelled','refunded'))"
    ...
    async def release_hold(self,ctx,channel_id:str,reason:str):
        ...
        async def confirm(inter):
            ...
            await db.query("UPDATE tracked_channels c SET hold=FALSE,closed_at=UTC_TIMESTAMP() WHERE channel_id=%s AND NOT EXISTS (SELECT 1 FROM orders o WHERE (o.channel_id=c.channel_id OR o.source_id=c.channel_id) AND o.status NOT IN ('completed','cancelled','refunded'))",(cid,))
