...
def components(embed,view,has_banner):
    children=[]
    if has_banner:
...
async def send(channel,embed,view,file):
    layout=components(embed,view,file is not None)
    ...
    data=await channel._state.http.send_files(channel.id,files=[file] if file else [],
